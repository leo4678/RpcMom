...
public class SimpleNettyChannelPool {
	...
	static {
		...
		.handler(new ChannelInitializer() {
			...
			protected void initChannel(Channel ch) throws Exception {
				...
				ch.pipeline().addLast("handler",new SimpleNettyChannelPoolHandler(NettyClient.getSimpleNettyChannelPool()));
						ch.pipeline().addLast(
	...
	public SimpleNettyChannelPool(SocketAddress remoteAddress, int maxConnNum,
	...
	public SimpleNettyResponseFuture sendRequest(Object request) throws InterruptedException,IOException{
		SimpleNettyResponseFuture responseFuture = new SimpleNettyResponseFuture();
	...
	private boolean sendRequestByPooledChannel(Object request,
			SimpleNettyResponseFuture responseFuture,boolean waiting) throws InterruptedException{
		...
		SimpleNettyResponseFutureUtil.attributeSimpleResponseFuture(channel, responseFuture);
	...
	private boolean sendRequestByNewChannel(Object request,
			SimpleNettyResponseFuture responseFuture, boolean forceConnect) throws InterruptedException {
	...
	public void returnChannel(Channel channel){
		if(SimpleNettyResponseFutureUtil.getForceConnect(channel)){
			return;
		}
		if(null != channel && channel.isActive()){
	...
	private void removeChannel(Channel channel, Throwable cause) {
		SimpleNettyResponseFutureUtil.cancel(channel, cause);
		if(!SimpleNettyResponseFutureUtil.getForceConnect(channel)){
	...
	private ChannelFuture createChannelFuture(boolean forceConnect){
		...
		if (null != future) {
			if(forceConnect){
				SimpleNettyResponseFutureUtil.attributeForceConnect(
